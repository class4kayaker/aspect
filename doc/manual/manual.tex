...
from a composition field, we instead initialize the problem by providing a
level set function.
...
\begin{figure}
    \centering
    \includegraphics[width=1.0\textwidth]{cookbooks/benchmarks/vof-van-keken/rms_vel_comparison.png}
    \caption{\it Van Keken benchmark: Evolution of the root mean square
        velocity as a function of time under different levels of mesh refinement.
        The lower refinement level model was run on a 128x128 mesh, matching the other
    models, while the higher refinement model was run on a 256x256 mesh.}
    \label{fig:vof-vk-1}
\end{figure}

We begin by examining the RMS velocity data shown in Figure~\ref{fig:vof-vk-1}, and note that the two
...
\begin{figure}
    ...
    \caption{\it Van Keken benchmark using VOF advection algorithm. The
        ...
        interfaces superimposed on the colorscale composition data for the higher
        refinement model. The contour for the higher refinement model is bright green,
    while the lower refinement contour is in black.}
    \label{fig:vof-vk-2}
\end{figure}

We begin by noting that the general structure of the solution shown in
Figure~\ref{fig:vof-vk-2} matches the expected form, and the general type of
structure shown in the other versions of this benchmark.
